use crate::utils::{
    function_header_span, iter_input_pats, match_def_path, qpath_res, return_ty, snippet,
    snippet_opt, span_help_and_lint, span_lint, span_lint_and_then, type_is_unsafe_function,
};
use matches::matches;
...
fn check_needless_must_use(
    ...
    } else if !attr.is_value_str() && !in_external_macro(cx.sess(), item_span) && is_must_use_ty(cx, return_ty(cx, item_id)) {
        span_help_and_lint(
            cx,
            DOUBLE_MUST_USE,
            function_header_span(cx, item_span),
